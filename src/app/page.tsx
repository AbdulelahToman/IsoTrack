...
import { CalendarDays, CirclePlus, Minus, Pill, Trash2 } from "lucide-react";
...
function Field({ label, unit, error, children }: { label: string; unit?: string; error?: string; children: React.ReactNode }) {
  return (
    <div className="space-y-1.5">
      ...
      {children}
      {error && <motion.p initial={{ opacity: 0, y: -2 }} animate={{ opacity: 1, y: 0 }} className="text-xs leading-5 text-red-600">{error}</motion.p>}
    </div>
  );
}
...
const SummaryMetric = memo(function SummaryMetric({ label, value, unit }: { label: string; value: string; unit: string }) {
  return (
    <div className="min-w-0 border-l-2 border-slate-200 pl-3 first:border-teal-500 sm:pl-4">
      <p className="text-xs font-medium leading-5 text-slate-500">{label}</p>
      <p className="mt-0.5 text-lg font-semibold tracking-[-0.025em] text-slate-950 sm:text-xl">{value} <span className="text-xs font-medium tracking-normal text-slate-400">{unit}</span></p>
    </div>
  );
});
...
function DosePeriodCard({ period, index, total, today, errors, focusDose, onChange, onRemove }: {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: 12, height: 0 }} transition={{ duration: 0.16 }} className="relative grid grid-cols-[16px_1fr] gap-3 pb-4 last:pb-0">
      {index < total - 1 && <motion.div initial={{ scaleY: 0 }} animate={{ scaleY: 1 }} className="absolute bottom-0 left-[8px] top-4 w-px origin-top bg-slate-200" />}
      <div className="relative z-10 mt-2 size-4 rounded-full border-[5px] border-[#f7f8fa] bg-teal-600 shadow-[0_0_0_1px_rgba(15,23,42,0.1)]" />
      <div className="rounded-xl border border-slate-200/80 bg-white/80 p-3.5 sm:p-4">
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            <p className="text-[11px] font-medium text-slate-400">Dose #{index + 1}</p>
        ...
        <div className="grid gap-3 sm:grid-cols-[0.7fr_1fr_1.2fr]">
          <Field label="Daily dose" unit="mg" error={errors.dose}><Input autoFocus={focusDose} aria-invalid={Boolean(errors.dose)} aria-label={`Dose period ${index + 1} daily dose`} min="1" type="number" value={period.dose} onChange={(event) => onChange(period.id, "dose", event.target.value === "" ? "" : Number(event.target.value))} /></Field>
...
export default function Home() {
  ...
  return (
    <main className="flex min-h-screen flex-col bg-[#f7f8fa] text-slate-950">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.22 }} className="mx-auto flex w-full max-w-5xl flex-1 flex-col px-4 py-4 sm:px-6 sm:py-6 lg:px-8">
        <header className="pb-3">
          <div className="flex items-center gap-2"><Pill className="size-4 text-teal-700" /><p className="text-base font-medium tracking-[-0.02em]">IsoTrack</p></div>
          <h1 className="mt-1 text-lg font-normal tracking-[-0.025em] text-slate-700 sm:text-xl">Isotretinoin Treatment Calculator</h1>
        </header>

        <nav aria-label="Section navigation" className="sticky top-0 z-30 -mx-4 border-y border-slate-200/90 bg-[#f7f8fa]/95 px-4 py-2 backdrop-blur-md sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8 print:hidden">
          <div className="flex gap-1"><a className="min-h-9 rounded-lg px-3 py-2 text-xs text-slate-600 transition-colors hover:bg-white hover:text-teal-700" href="#planning">Planning</a><a className="min-h-9 rounded-lg px-3 py-2 text-xs text-slate-600 transition-colors hover:bg-white hover:text-teal-700" href="#history">History</a><a className="min-h-9 rounded-lg px-3 py-2 text-xs text-slate-600 transition-colors hover:bg-white hover:text-teal-700" href="#summary">Summary</a></div>
        </nav>
        ...
        <div className="grid items-start gap-8 pt-5 lg:grid-cols-[0.78fr_1.22fr] lg:gap-10">
          <motion.section id="planning" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.03, duration: 0.2 }} aria-label="Treatment Planning" className="scroll-mt-14">
            <div className="space-y-3.5">
              <Field label="Weight" unit="kg" error={weight !== "" && weight <= 0 ? "Enter a weight greater than 0 kg." : undefined}><Input aria-invalid={weight !== "" && weight <= 0} aria-label="Weight in kilograms" min="1" placeholder="Enter weight (kg)" type="number" value={weight} onChange={(event) => setWeight(event.target.value === "" ? "" : Number(event.target.value))} /></Field>
              <Field label="Planned daily dose" unit="mg/day" error={plannedDose !== "" && plannedDose <= 0 ? "Enter a dose greater than 0 mg." : undefined}><Input aria-invalid={plannedDose !== "" && plannedDose <= 0} aria-label="Planned daily dose" min="1" type="number" value={plannedDose} onChange={(event) => setPlannedDose(event.target.value === "" ? "" : Number(event.target.value))} /></Field>
            ...
            <div className="mt-4 rounded-xl border border-slate-200 bg-white p-3.5">
              <p className="text-[11px] text-slate-500">Target cumulative dose</p><p className="mt-0.5 text-lg font-medium tracking-tight">{numericWeight > 0 ? `${target120.toLocaleString()}–${target150.toLocaleString()} mg` : "—"}</p>
              <div className="mt-3 grid grid-cols-2 gap-3 border-t border-slate-100 pt-3 text-[11px]"><div><p className="text-slate-400">Reach 120 mg/kg</p><p className="mt-1 text-teal-700">{formatDate(plannedEnd120)}</p></div><div><p className="text-slate-400">Reach 150 mg/kg</p><p className="mt-1 text-teal-700">{formatDate(plannedEnd150)}</p></div></div>
            </div>
          </motion.section>
          ...
          <motion.section id="history" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.06, duration: 0.2 }} aria-labelledby="history-title" className="scroll-mt-14">
            <div className="mb-4 border-b border-slate-200 pb-2.5"><h2 id="history-title" className="text-base font-semibold tracking-tight">Current cumulative dose</h2></div>
            ...
            <Button onClick={addDosePeriod} variant="ghost" className="mt-2 h-11 w-full justify-start rounded-lg text-teal-700 print:hidden"><CirclePlus /> Dose changed</Button>

            <section aria-labelledby="progress-title" className="mt-6 border-t border-slate-200 pt-5">
              <div className="flex items-center justify-between"><h3 id="progress-title" className="text-sm font-semibold">Progress</h3><span className="text-[11px] text-slate-400">mg/kg</span></div>
              <div className="relative mb-1 mt-10 h-1.5 rounded-full bg-slate-300">
                ...
                <motion.div animate={{ left: `${rulerPosition}%` }} transition={{ type: "spring", stiffness: 220, damping: 27 }} className="absolute top-1/2 z-10 -translate-x-1/2 -translate-y-1/2">
                  ...
                  <div className={`mx-auto size-4 rounded-full border-[3px] border-white shadow-sm ${progressTone === "target" ? "bg-emerald-600" : progressTone === "high" ? "bg-orange-500" : "bg-slate-700"}`} />
                </motion.div>
              </div>
              <div className="grid grid-cols-[1fr_auto_20%] text-[10px] text-slate-500"><span>0</span><span className="text-emerald-700">120</span><span className="text-right">150</span></div>
            </section>

            <section id="summary" aria-labelledby="summary-title" className="mt-5 scroll-mt-14 border-t border-slate-200 pt-5">
              ...
              <p className="mb-2 mt-5 text-[11px] text-slate-500">Estimated days remaining · 120 / 150 mg/kg</p>
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                {doseOptions.map((dose) => <div key={dose} className="rounded-lg border border-slate-200 bg-white px-3 py-2"><p className="text-[11px] text-slate-500">{dose} mg/day</p><p className="mt-0.5 flex items-center gap-1.5 text-sm font-medium tabular-nums">{numericWeight > 0 ? <><span>{Math.ceil(remaining120 / dose)}</span><Minus className="size-3 text-slate-300" /><span>{Math.ceil(remaining150 / dose)} d</span></> : <span>—</span>}</p></div>)}
              </div>
            </section>
        ...
        <footer className="mt-auto pb-1 pt-12 text-center text-[11px] text-slate-400 sm:pt-16">© By Abdulelah Alghamdi</footer>
