...
function DoseSelect({ value, label, autoFocus, invalid, onChange }: {
  ...
  return (
    <select
      ...
      aria-label={label}
      autoFocus={autoFocus}
      className="h-11 w-full max-w-full min-w-0 box-border rounded-lg border border-input bg-white px-3 text-base shadow-xs outline-none transition-[color,background-color,box-shadow,border-color] focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/20 dark:bg-slate-900 dark:text-slate-100 dark:[color-scheme:dark] sm:text-sm"
      value={value}
  ...
  return (
    <div className="min-w-0 space-y-1.5">
      <div className="flex items-center justify-between gap-2">
        <Label>{label}</Label>
        {unit && <span className="text-[11px] font-medium text-slate-400 dark:text-slate-500">{unit}</span>}
      </div>
      {children}
...
const SummaryMetric = memo(function SummaryMetric({ label, value, unit }: { label: string; value: string; unit: string }) {
  return (
    <div className="min-w-0 border-l-2 border-slate-200 pl-3 first:border-teal-500 dark:border-slate-700 sm:pl-4">
      <p className="text-xs font-medium leading-5 text-slate-500 dark:text-slate-400">{label}</p>
      <p className="mt-0.5 text-lg font-semibold tracking-[-0.025em] text-slate-950 dark:text-slate-100 sm:text-xl">{value} <span className="text-xs font-medium tracking-normal text-slate-400 dark:text-slate-500">{unit}</span></p>
    </div>
  );
});
...
function DosePeriodCard({ period, index, total, today, errors, focusDose, onChange, onRemove }: {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: 12, height: 0 }} transition={{ duration: 0.16 }} className="relative grid min-w-0 grid-cols-[16px_minmax(0,1fr)] gap-3 pb-4 last:pb-0">
      {index < total - 1 && <motion.div initial={{ scaleY: 0 }} animate={{ scaleY: 1 }} className="absolute bottom-0 left-[8px] top-4 w-px origin-top bg-slate-200 dark:bg-slate-700" />}
      <div className="relative z-10 mt-2 size-4 rounded-full border-[5px] border-[#f7f8fa] bg-teal-600 shadow-[0_0_0_1px_rgba(15,23,42,0.1)] dark:border-[#111923] dark:shadow-[0_0_0_1px_rgba(148,163,184,0.25)]" />
      <div className="min-w-0 rounded-xl border border-slate-200/80 bg-white/80 p-3.5 dark:border-slate-700/80 dark:bg-slate-800/80 sm:p-4">
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            <p className="text-[11px] font-medium text-slate-400 dark:text-slate-500">Dose #{index + 1}</p>
    ...
    <main className="flex min-h-screen min-w-0 flex-col overflow-x-hidden bg-[#f7f8fa] text-slate-950 transition-colors duration-150 dark:bg-[#111923] dark:text-slate-100">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.22 }} className="mx-auto flex w-full max-w-5xl flex-1 flex-col px-4 py-4 sm:px-6 sm:py-6 lg:px-8">
        <header className="flex items-start justify-between gap-3 pb-3">
          ...
          <h1 className="mt-1 text-lg font-normal tracking-[-0.025em] text-slate-700 dark:text-slate-300 sm:text-xl">Isotretinoin Treatment Calculator</h1></div>
          <ThemeToggle />
        </header>

        <nav aria-label="Section navigation" className="sticky top-0 z-30 -mx-4 border-y border-slate-200/90 bg-[#f7f8fa]/95 px-4 py-2 backdrop-blur-md transition-colors duration-150 dark:border-slate-700/90 dark:bg-[#111923]/95 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8 print:hidden">
        ...
        <div className="grid items-start gap-8 pt-5 lg:grid-cols-[0.78fr_1.22fr] lg:gap-10">
          <motion.section id="planning" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.03, duration: 0.2 }} aria-label="Treatment Planning" className="scroll-mt-14">
            ...
            <div className="mt-4 rounded-xl border border-slate-200 bg-white p-3.5 dark:border-slate-700 dark:bg-slate-800">
              <p className="text-[11px] text-slate-500 dark:text-slate-400">Target cumulative dose</p><p className="mt-0.5 text-lg font-medium tracking-tight">{numericWeight > 0 ? `${target120.toLocaleString()}–${target150.toLocaleString()} mg` : "—"}</p>
              <div className="mt-3 grid grid-cols-2 gap-3 border-t border-slate-100 pt-3 text-[11px] dark:border-slate-700"><div><p className="text-slate-400 dark:text-slate-500">Reach 120 mg/kg</p><p className="mt-1 text-teal-700 dark:text-teal-400">{formatDate(plannedEnd120)}</p></div><div><p className="text-slate-400 dark:text-slate-500">Reach 150 mg/kg</p><p className="mt-1 text-teal-700 dark:text-teal-400">{formatDate(plannedEnd150)}</p></div></div>
            </div>
          </motion.section>
          ...
          <motion.section id="history" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.06, duration: 0.2 }} aria-labelledby="history-title" className="scroll-mt-14">
            <div className="mb-4 border-b border-slate-200 pb-2.5 dark:border-slate-700"><h2 id="history-title" className="text-base font-semibold tracking-tight">Current cumulative dose</h2></div>
            ...
            <section aria-labelledby="progress-title" className="mt-6 border-t border-slate-200 pt-5 dark:border-slate-700">
              <div className="flex items-center justify-between"><h3 id="progress-title" className="text-sm font-semibold">Progress</h3><span className="text-[11px] text-slate-400 dark:text-slate-500">mg/kg</span></div>
              <div className="relative mb-1 mt-10 h-1.5 rounded-full bg-slate-300 dark:bg-slate-600">
                ...
                <motion.div animate={{ left: `${rulerPosition}%` }} transition={{ type: "spring", stiffness: 220, damping: 27 }} className="absolute top-1/2 z-10 -translate-x-1/2 -translate-y-1/2">
                  ...
                  <div className={`mx-auto size-4 rounded-full border-[3px] border-white shadow-sm dark:border-slate-900 ${progressTone === "target" ? "bg-emerald-600" : progressTone === "high" ? "bg-orange-500" : "bg-slate-700"}`} />
                </motion.div>
              </div>
              <div className="grid grid-cols-[1fr_auto_20%] text-[10px] text-slate-500 dark:text-slate-400"><span>0</span><span className="text-emerald-700 dark:text-emerald-400">120</span><span className="text-right">150</span></div>
            </section>

            <section id="summary" aria-labelledby="summary-title" className="mt-5 scroll-mt-14 border-t border-slate-200 pt-5 dark:border-slate-700">
              ...
              <p className="mb-2 mt-5 text-[11px] text-slate-500 dark:text-slate-400">Estimated days remaining · 120 / 150 mg/kg</p>
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                {doseOptions.map((dose) => <div key={dose} className="rounded-lg border border-slate-200 bg-white px-3 py-2 dark:border-slate-700 dark:bg-slate-800"><p className="text-[11px] text-slate-500 dark:text-slate-400">{dose} mg/day</p><p className="mt-0.5 flex items-center gap-1.5 text-sm font-medium tabular-nums">{numericWeight > 0 ? <><span>{Math.ceil(remaining120 / dose)}</span><Minus className="size-3 text-slate-300 dark:text-slate-600" /><span>{Math.ceil(remaining150 / dose)} d</span></> : <span>—</span>}</p></div>)}
              </div>
            </section>
        ...
        <footer className="mt-auto pb-1 pt-12 text-center text-[11px] text-slate-400 dark:text-slate-500 sm:pt-16">© By Abdulelah Alghamdi</footer>
